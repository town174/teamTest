...
private void Update_member_and_Reserves(int Points,string User_Name)
 {
     ...
     sql="insert into pa_fnc_list(site_id,thetype,username,act,amount,detail,beizhu) values("+int.Parse(SiteId)+",2,'"+Sql_Format(User_Name)+"','k',"+Points+",'兑换商品','"+Sql_Format(Lb_Title.Text)+"("+Enum+")')";
     ...
     sql="insert into pa_exchange(site_id,thetable,detail_id,username,title,num,paypoint,name,tel,email,address,postcode,operator) values("+int.Parse(SiteId)+",'"+Sql_Format(Table)+"',"+int.Parse(DetailId)+",'"+Sql_Format(User_Name)+"','"+Sql_Format(Lb_Title.Text)+"',"+int.Parse(Enum)+","+Points+",'"+Sql_Format(Request.Form["truename"])+"','"+Sql_Format(Request.Form["tel"])+"','"+Sql_Format(Request.Form["email"])+"','"+Sql_Format(Request.Form["address"])+"','"+Sql_Format(Request.Form["postcode"])+"','')";
     comm=new OleDbCommand(sql,conn);
     comm.ExecuteNonQuery();
 }
