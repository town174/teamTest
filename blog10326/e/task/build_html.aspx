 ...
 protected void Page_Load(Object src,EventArgs e)
  {
    ...
    if(IsStr(Table) && IsStr(Ids.Replace(",","")))
    {
    ...
    {
      Response.Write("请设置好参数,参数类型：tables=表名&ids=id组(多个id用,号隔开)");
    }
  }

private void Build_Zt()
 {
    conn.Open();
    if(Ids!=null && IsNum(Ids.Replace(",","")))
     {
       ...
       if(IsNum(Ids))
        {
         sql="select site_dir,id,zt_dir,zt_file from "+Table+" where id="+Ids;
        }
       else
        {
         sql="select site_dir,id,zt_dir,zt_file from "+Table+" where id in("+Ids+")";
        }
       ...
       while(dr.Read()) 
        {
          try
           {
            BH.Build_Zt(dr["site_dir"].ToString(),dr["zt_dir"].ToString(),dr["zt_file"].ToString(),dr["id"].ToString());
           }
          catch(Exception e)
          {
            LocalUrl="http://"+Request.ServerVariables["SERVER_NAME"]+":"+Request.ServerVariables["SERVER_PORT"];
            ErrorUrl=LocalUrl+"/e/zt/index.aspx?id="+dr["id"].ToString();
    ...
    if(Ids!=null && IsNum(Ids.Replace(",","")))
     {
      ...
      dr.Close();
      conn.Close();
      Response.Write("生成"+Table+"表内容页成功!");
      Response.End();
     }
 }
        ...
        if(dr["thetable"].ToString()!="zdy" && dr["thetable"].ToString()!="" && dr["list_html"].ToString()=="2") //生成静态page页
         {
            ...
            for(int i=1;i<=Page_Num;i++)
             {
               BH.Build_SubLanmu(1,dr["site_id"].ToString(),dr["site_dir"].ToString(),dr["lanmu_id"].ToString(),dr["lanmu_dir"].ToString(),dr["id"].ToString(),dr["parent_dir"].ToString(),dr["sublanmu_dir"].ToString(),"index_"+i+".html",dr["thetable"].ToString(),i);
             }
         }
