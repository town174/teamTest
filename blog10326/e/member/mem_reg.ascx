...
<tr><td colspan="2" align="center"><input id="bt_post" type="button"  value=" 提示：请先填写以上注册资料！ " onclick="return Check_Reg('<%=security_code%>')" disabled style="width:300px;"><td></tr>
...
<asp:TextBox Id="Bt_Email" runat="server" cssclass="tb"/> <asp:Button OnClick="ChangeActMail" Text="发 送" class="m_bt" runat="server"/>
</div>
<div style="padding-top:10px"><input type="button" class="m_bt" value="转到登录页面" onclick="location.href='<%=GetUrl("login")%>'"></div>
</div>
...
<asp:label id="lb_fieldrepeat"  runat="server" visible="false">对不起，<%=RepeatField%>的数据已经存在，请重新输入并保证此值的唯一性。</asp:Label>
