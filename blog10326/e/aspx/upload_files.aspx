...
<script Language="C#" Runat="server">
   string UserName,Table,Field,FieldType,Post,TheTitle,Url,T_Url,FileSize,Content,Permissions,Point,Xuhao,IsMaster,Thumbnail_MinWidth,Thumbnail_MinHeight,MaxNum,Allow_Upload,The_Ext,The_Maxlength;
   string M_Type,DepartmentId,sql;
   int SiteId,InforId;
   ...
   protected void Page_Load(Object sender,EventArgs e)
    {
     if(Request.QueryString["from"]=="master")
      ...
      conn.Open();
        ...
        if(IsMaster=="0")
         {
          Get_Perrmission(int.Parse(DepartmentId),int.Parse(M_Type));
         }
        else
         {
          Allow_Upload="1";
...
private void Get_Perrmission(int DId,int MtypeId)
 {
    ...
    OleDbCommand comm;
    OleDbDataReader dr;
     if(Table=="pa_member") //如果是管理员判断是否管理员组
      {
...
private void Get_Member_Type(int MtypeId)
 {
   string sql="select * from pa_member_type order by xuhao";
...
<style type="text/css">
body{word-wrap:break-word;font:12px/1.6em Tahoma,Helvetica,Arial,sans-serif;color:#333333;font-family:inherit;background-color:#ffffff;padding:0px;margin:10px auto 0px auto;width:540px;}
...
<td align=left><input type="textbox" id="url" name="url" size="40"  maxlength="200" ondblclick="ViewUrl(this.value)" value="<%=Url%>" <%=(Url.ToLower().IndexOf("http://")==0 || Url=="")?"":"readonly onkeydown=\"alert('禁止直接修改!');return false\""%> title="格式如：http://www.pageadmin.net/image.jpg"><%if(IsMaster=="1"){%>&nbsp;<input type="button" id="bt_url" value="浏览服务器" onclick="open_select('','<%=FieldType%>','<%=Request.QueryString["table"]%>','<%=Request.QueryString["field"]%>','url')" style="height:22px;"><%}%></td>
</tr>
...
<td align=left>
  <input type="checkbox" name="Visiter_all" id="Visiter_all" value="0"  onclick="select_all()">无限制<br>
        <asp:Repeater id="P_permissions" runat="server">
         <ItemTemplate>
           <input type="checkbox" name="Visiter" id="Visiter_<%#DataBinder.Eval(Container.DataItem,"id")%>" value="<%#DataBinder.Eval(Container.DataItem,"id")%>" onclick="Get_Visiter()"><%#DataBinder.Eval(Container.DataItem,"name")%>&nbsp;
         </ItemTemplate>
        </asp:Repeater>
</td>
</tr>
...
</div><div class="main" name="tabcontent" id="tabcontent" style="padding:0px;">
...
<input type="hidden" id="swf_permissions" value="<%=Permissions%>">
...
var swf_permissions=document.getElementById("swf_permissions");//swf附件组会员权限
function Get_Visiter()
 {
   var val="";
   var Visiter=document.getElementsByName("Visiter");
   for(i=0;i<Visiter.length;i++)
    {
      if(Visiter[i].checked)
       {
        val+=","+Visiter[i].value;
       }
    }
   ...
   swf_permissions.value=val;
 }

function select_all()
 {
   var obj=document.getElementById("Visiter_all");
   if(obj.checked)
    {
      lock_mem_check();
      swf_permissions.value="0";
    }
   else
    {
     unlock_mem_check();
