...
<%=Sorts%>
 ...
 <tr>
   <td align=left>
   <select name="F_Name" id="F_Name" style="width:98%;height:250px;" multiple  ondblclick="Get_Friend(this.options[this.selectedIndex].value,this.options[this.selectedIndex].text)" title="可以通过双击选中，也可以按住Ctrl键实现多选后按确定选择">
<%=Friends%>
  </select><br>共<%=count%>条记录。
  </td>
 </tr>
     ...
     {   
      if(Obj.options[i].selected)
      {
       if(!CheckRepeat(Obj.options[i].value))
        {
         parent.AddUsers(Obj.options[i].text,Obj.options[i].value);
        }
      }
     }
   wclose();
...
function Get_Friend(id,txt)
 {
  if(!CheckRepeat(id))
    {
      parent.AddUsers(txt,id);
    }
 }

var op_obj=parent.document.getElementById("receiver_users");
