...
<script language="c#" runat="server">
 string SiteId,Table,SortId,SortIds,SublanmuId,Rv,sql;
 ...
 protected void Page_Load(Object src,EventArgs e)
  {
    ...
    if(IsNum(SiteId) && IsNum(SortId) && IsNum(SublanmuId) && IsStr(Table))
    {
...
private void find_allsublanmu() 
 {
    OleDbCommand comm;
    OleDbDataReader dr;
    sql="select id from pa_sublanmu where site_id="+SiteId+" and thetable='"+Table+"' and sort_id>=1 and is_sortsublanmu=1 and model_id>=1 and id<>"+SublanmuId; 
...
private void find_sublanmu(string SortIds) 
 {
    ...
    if(IsNum(SortIds))
     {
      sql="select id from pa_sublanmu where site_id="+SiteId+" and thetable='"+Table+"' and sort_id="+SortIds+" and is_sortsublanmu=1 and model_id>=1 and id<>"+SublanmuId; 
      comm=new OleDbCommand(sql,conn);
      dr=comm.ExecuteReader();
      if(dr.Read())
      {
