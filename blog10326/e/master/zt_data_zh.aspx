...
<script Language="c#" Runat="server">
  ...
  int Site_Id,PageSize,List_Level,Is_Static,Is_Static_1;
  string Ismultiple,SiteDir,TheTable,List_Space,List_style,sql_str,order_str,Url_Prefix,Url_Prefix_1,TheMaster,Site_List,Table_List,Sort_List,ZtId;
   ...
   if(!Page.IsPostBack)
    {
      ...
      ViewState["Calculatesql"]="select count(id) as co from "+TheTable+" where checked=1 and source_id=0 "+sql_str;
      ViewState["sql"]="select id from "+TheTable+" where checked=1 and source_id=0 "+sql_str+"  order by "+order_str;
...
private DataSet Optimize_DataSet(DataSet SourceDs)
 {
   ...
   SourceDs.Clear();
  string sql="select id,title,thedate,username,site_id,static_dir,static_file,lanmu_id,sublanmu_id,zdy_url,permissions,html,checked,zt_ids,istg,sort_id from "+TheTable+" where id in("+Ids+")";
   ...
   switch(Act)
    {
       ...
       for(int i=0;i<Aids.Length;i++)
        {
          ...
          {  
           ...
           sql="update "+TheTable+" set iszt=1,zt_ids='"+IDS+"' where id="+Aids[i];
           comm=new OleDbCommand(sql,conn);
           comm.ExecuteNonQuery();
          }
          dr.Close();
        }
      log.Save(int.Parse(Request.Cookies["SiteId"].Value),1,"edit",1,TheMaster,"添加专题信息");
      break;
      ...
      log.Save(int.Parse(Request.Cookies["SiteId"].Value),1,"edit",1,TheMaster,"移动专题信息");
      break;
    }
  conn.Close();
...
<div id="tabdiv">
<ul>
<li id="tab" name="tab" onclick="location.href='zt_data.aspx?id=<%=Request.QueryString["id"]%>&table=<%=Request.QueryString["table"]%>'">专题信息</li>
<li id="tab" name="tab" style="font-weight:bold">组合专题</li>
</ul>
</div>
<form runat="server">
...
搜索：<select name="field" id="field" onchange="Go()">
          ...
          <asp:Repeater id="P1" runat="server" OnItemDataBound="Data_Bound">         
             <ItemTemplate>
                 <tr class="listitem">
                  ...
                  <td align=center class=tdstyle title="√表示已经加入到了本专题，×表示未加入到本专题"><img src='/e/images/public/<%#DataBinder.Eval(Container.DataItem,"zt_ids").ToString().IndexOf(","+ZtId+",")>=0?"g.gif":"x.gif"%>' border=0></td>
                  <td align=center class=tdstyle><input type="button" class="button" onclick="singleset(this.value,<%#DataBinder.Eval(Container.DataItem,"id")%>)" <%#DataBinder.Eval(Container.DataItem,"zt_ids").ToString().IndexOf(","+ZtId+",")>=0?" value='移除'":" value='加入'"%>></td>
                </tr>
             </ItemTemplate>
          </asp:Repeater>
        <tr>
...
<a href="javascript:set('add')">[添加到本专题]</a>
<a href="javascript:set('del')">[从本专题移除]</a>
</td>
