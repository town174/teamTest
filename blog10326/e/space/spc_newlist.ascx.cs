   ...
   if(UserName!="" && tables!="")
    {
     ...
     for(int i=0;i<Atables.Length;i++)
     {
      ...
      sql="select top 15 id,site_id,sort_id,id,title,lanmu_id,sublanmu_id,thedate,static_dir,Static_file,zdy_url,permissions,clicks,[html],thedate,'"+Atables[i]+"' as thetable from "+Atables[i]+" where sublanmu_id>0 and checked=1 and username='"+UserName+"' order by thedate desc";
