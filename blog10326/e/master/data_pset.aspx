...
<script language="c#" Runat="server">
  ...
  OleDbCommand comm;
  OleDbDataReader dr;
  string Thetable,PushFields,Fields,Fields_Name,SiteList,CurrentSiteId,Sort_List,TheMaster,field_value,sql,sql_1,Sort_Ids,Admin_Sites,ToSite_Dir;
  int SiteId,Lanmu_Id,Sublanmu_Id,Detail_Html;
   ...
   if(!Page.IsPostBack)
    {
       ...
       ViewState["constr"]=connstr.Constr();//获取连接字符串
...
private void MemberType_Bind()
 {
   ...
   P_permissions.DataSource= ds.Tables["table"].DefaultView;
   P_permissions.DataBind();
   P_DownLoad.DataSource= ds.Tables["table"].DefaultView;
   P_DownLoad.DataBind();
 }
...
private void Get_AdminSites()
 {
   sql="select admin_sites from pa_member where username='"+TheMaster+"'";
   comm=new OleDbCommand(sql,conn);
   dr=comm.ExecuteReader();
   if(dr.Read())
    {
...
private void Data_Update()
 {
   ...
   if(Request.Form["Visiter_all"]=="0")
    {
     permissions="";
    }
   else
    {
     permissions=Request.Form["Visiter"];
    }
   if(Request.Form["download_all"]=="0")
    {  
     download_permissions="0";
    }
   else
    {
     download_permissions=Request.Form["download"];
    }
...
private void Add_PushData(int detailid)
 {
    ...
    string TheFields=PushFields+",[lanmu_id],[sublanmu_id],[html],[site_dir],[static_dir],[detail_model_id],[zdy_title],[zdy_keywords],[zdy_description],[zdy_url],[member_price],[reserves],[clicks],[downloads],[comments],[checked],[iscg],[istg],[istop],[isgood],[isnew],[ishot],[comment_open],[comment_check],[comment_anonymous],[tags],[related_ids],[permissions],[point],[sendpoint],[username],[reply_state],[ip],[actdate]";
...
<td>
  <input type="checkbox" name="download_all" id="download_all" value="0"  onclick="select_all('download')">无限制<br>
        <asp:Repeater id="P_DownLoad" runat="server">
         <ItemTemplate>
           <input type="checkbox" name="download" id="download_<%#DataBinder.Eval(Container.DataItem,"id")%>" value="<%#DataBinder.Eval(Container.DataItem,"id")%>" ><%#DataBinder.Eval(Container.DataItem,"name")%>&nbsp;
         </ItemTemplate>
        </asp:Repeater>
</td>
</tr>
...
<td>
<input type="button" value="选择分类" class="bt" onclick="sort_Select();">
</td></tr>
</table>
</td>
 </tr>
...
<input type="submit" class=button value="提交" onclick="return ck()" onmouseover="set_selected('zt_list',true);set_selected('push_list',true);">
...
function sort_Select()
 {
  IDialog("推送分类","sort_select.aspx?table=<%=Request.QueryString["table"]%>",650,380);
 }
