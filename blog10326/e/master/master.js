...
function open_departmentuser(formtype,openerid,tj,title)
 {
  IDialog(title,"department_user.aspx?type="+formtype+"&id="+openerid+"&condition="+encodeURI(tj),280,360);
 }
...
function Issue_Data(Table,DetailId,Wid,xuhao) //签发信息
 {
    var Url="/e/member/issue.aspx?table="+Table+"&detailid="+DetailId+"&workid="+Wid+"&ismaster=1&xuhao="+xuhao;
...
function open_ztlist(ismultiple)
 {
  IDialog("选择专题","zt_select.aspx?multiple="+ismultiple,700,"90%");
 }
...
function open_upload(sid,path,type,table,field,ObjId) //上传界面
 {
  ...
  IDialog("文件上传","/e/aspx/upload_panel.aspx?sid="+sid+"&filepath="+path+"&type="+type+"&table="+table+"&field="+field+"&objid="+ObjId+"&from=master",Width,Height);
}
...
function GetIframeContent()
 {
     var doc;
     if (document.all){//IE  
                if(document.frames["post_iframe"]==null){return;}
                doc = document.frames["post_iframe"].document;  
        }else{//Firefox or Chrome   
                if(document.getElementById("post_iframe")==null){return;}
                doc= document.getElementById("post_iframe").contentDocument; 
        } 
     if(doc==null){return;}
     ...
     if(content.length>0)
      {
        ...
        startpost_viewIframeOver=1;
      }
     if(startpost_viewIframeOver==0)
      {
       setTimeout(GetIframeContent,3000);
      }
 }  
       ...
       for(i=0;i<AFields.length-1;i++)
        {
          ...
          if(Obj.offsetHeight==0 || Obj==null){continue;}
          switch(AFieldtype[i])
           {
             case "select":
                J_start="请选择";
