   ...
   public class PaInstall:Page
    {
      ...
      protected PlaceHolder Panel1,Panel2,Panel3,Panel4,Panel5,P_Sql;
     ...
     protected void Show_Sql(Object src,EventArgs e)
      {
        if(check_Installlock())
        {
         P_Sql.Visible=true;
     ...
     protected void Update_Sql(Object src,EventArgs e)
      {
        if(!check_Installlock())
        {
         return;
       ...
       if(Tb_sql.Text.Trim()=="")
       {
        Lbl_error.Text="提示：请输入sql运行语句!";
        return;
       }
      else
      {
       string[] Asql=Tb_sql.Text.Split(';');
       Conn conn1=new Conn();
       string Str_Constr=conn1.Constr();
       OleDbConnection conn=new OleDbConnection(Str_Constr);
       conn.Open();
       OleDbCommand comm;
          ...
          if(Asql[i].Trim()!="")
           {
            try
            {
              comm=new OleDbCommand(Asql[i].Trim(),conn);
              comm.ExecuteNonQuery();
            }
            catch(Exception ee)
            {
              Run_Result+=ErrorNum.ToString()+":"+ee.Message+"<br>";
              ErrorNum++;
            }
          }
       }
      ...
      if(Run_Result!="")
       {
        Lbl_error.Text="出现以下错误(字段已经存在的错误可以忽略)：<br>"+Run_Result+"<br>";
       }
      else
       {
        Lbl_error.Text="您好，sql运行成功!<br>";
       }
       Build_Installlock();
      }
     }
...
private bool Update_DataBase()
 {  
   ...
   OleDbConnection Myconn=new OleDbConnection(Str_Constr);
   Myconn.Open();
