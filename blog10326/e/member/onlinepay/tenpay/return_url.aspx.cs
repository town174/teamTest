		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(resHandler.isTenpaySign()) 
			{
				...
				string pay_result = resHandler.getParameter("pay_result");
	                       ...
	                       string zdy_info=resHandler.getParameter("attach"); //会员和site组合
				...
				if( "0".Equals(pay_result) ) 
				{
					...
					resHandler.doShow("http://"+url+"/e/member/onlinepay/tenpay/show.aspx?s="+SiteId+"&total_fee="+Fnc_Amount+"&sp_billno="+sp_billno);
				} 
				else 
				{
					//当做不成功处理
...
public void Update_fnc_list(double Fnc_Amount,string Rk_way,string OrderId,string Beizhu)
 {
     OrderId="tenpay_"+OrderId;
     string sql="insert into pa_fnc_list([site_id],[thetype],[username],[act],[amount],[detail],[identification],[beizhu]) values("+SiteId+",1,'"+Sql_Format(UserName)+"','c',"+Fnc_Amount+",'"+Rk_way+"','"+Sql_Format(OrderId)+"','"+Beizhu+"')";
     OleDbCommand Comm=new OleDbCommand(sql,conn);
     Comm.ExecuteNonQuery();
