...
<table border=0 cellpadding=5 cellspacing=0  style="table-layout:fixed;align:center;">
    <tr>
      <td align="right" width="70px">账户余额：</td>
     <td align="left"><span style="color:#ff0000;Font-Size:13px;font-weight:bold">￥<%=Fnc_Ky%></span></td>
    </tr>
    <tr>
      <td align="right">充值金额：</td>
     <td align="left"><input id="amount" type=text maxlength="8" value="<%=Request.QueryString["amount"]%>" size="10" onkeyup="if(isNaN(value))execCommand('undo')">&nbsp;元
     </td>
    </tr>
    <tr>
    ...
    <tr>
      <td align="left" colspan="2" style="padding-left:70px"><input type="button" class="m_bt" value="充 值" onclick="GoToPay()"></td>
    </tr>
    <tr>
     <td colspan="2" align=left style="padding-left:20px"><span style="color:#ff0000">提示：</span>支付未结束前请不要关闭浏览器，支付成功后该充值款项将划入您的会员帐户上。</td>
    </tr>
</TABLE>
</div>
</div>
 ...
 { 
  ...
  for(var i=0;i<pay_type.length;i++)
   ...
   switch(P_type)
    {
      case "alipay":
       window.open("/e/member/onlinepay/alipay/Default.aspx?s=<%=SiteId%>&amount="+M,"onlinepay")
      break;

      case "chinabank":
       window.open("/e/member/onlinepay/chinabank/Default.aspx?s=<%=SiteId%>&amount="+M,"onlinepay")
      break;

      case "tenpay":
       window.open("/e/member/onlinepay/tenpay/index.aspx?s=<%=SiteId%>&amount="+M,"onlinepay")
      break;
    }
 }
 </script>