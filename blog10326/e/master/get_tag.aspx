...
private void Table_Bind()
 {
   ...
   D_Table.DataBind();
   D_Table.Items.Insert(0,new ListItem("---请选择模型对应表---","0"));
 }
   ...
   switch(TagType)
    {
      case "sublanmu":
        model_name="---请选择子栏目模型---";
        sql="select * from pa_model where [language] in('"+P_Language+"','all') and thetable='"+TheTable+"' and thetype='sublanmu' and hasfile=1 order by xuhao";
      break;

      case "nav":
       model_name="---请选择导航模型---";
       sql="select * from pa_model where [language] in('"+P_Language+"','all') and thetable='"+TheTable+"' and thetype='nav' and hasfile=1 order by xuhao";
      break;

      case "module":
        model_name="---请选择模块模型---";
        sql="select * from pa_model where [language] in('"+P_Language+"','all') and thetable='"+TheTable+"' and thetype='module' and hasfile=1 order by xuhao";
      break;
...
<input type="button" value="选择专题" class="bt" onclick="open_ztlist(0)"> 注：表示只调用属于对应专题的信息，不选择则不限制
</td>
</tr>
