...
private void Get_Data()
  {
    ...
    if(IsStr(Table) && IsStr(Field) && IsStr(FieldType))
    {
      ...
      if(InforId==0)
       {
          sql="select id,title,url,thumbnail,thetable,field,detail_id,username,xuhao from pa_file where username='"+UserName+"' and thetable='"+Table+"' and field='"+Field+"' and detail_id="+InforId+" order by xuhao,id";
       }
      else
       {
         sql="select id,title,url,thumbnail,thetable,field,detail_id,username,xuhao from pa_file where thetable='"+Table+"' and field='"+Field+"' and detail_id="+InforId+" order by xuhao,id";
       }
...
<style type=text/css>
body div td{margin:0px;padding:0px;word-wrap:break-word;text-align:center;font:12px/20px Verdana,Helvetica,Arial,sans-serif;color:#333333;}
...
<form Runat="server">
  <asp:DataList id="Image_List" RepeatColumns="5" RepeatDirection="Horizontal" runat="server" border=0 RepeatLayout="Table" cellspacing=0 cellpadding=0 OnItemDataBound="Data_Bound"  OnDeleteCommand="Data_Delete">
   <ItemTemplate>
    <a href='<%#DataBinder.Eval(Container.DataItem,"url")%>' target="url"><img src="<%#DataBinder.Eval(Container.DataItem,"thumbnail")%>" border=0 width=110px height=100px></a><br><asp:TextBox id="Tb_title" Text='<%#DataBinder.Eval(Container.DataItem,"title")%>' Runat="server" style="width:110px;height:16px;border:1px solid #999999" Maxlength="50" /><br>序号<asp:TextBox id="Tb_xuhao" Text='<%#DataBinder.Eval(Container.DataItem,"xuhao")%>' Runat="server"  style="width:25px;height:16px;border:1px solid #999999" Maxlength="5"/>&nbsp;<a href="javascript:Edit_File('<%#DataBinder.Eval(Container.DataItem,"id")%>','<%#DataBinder.Eval(Container.DataItem,"thetable")%>','<%#DataBinder.Eval(Container.DataItem,"field")%>','<%=FieldType%>','<%#DataBinder.Eval(Container.DataItem,"detail_id")%>','<%=SiteId%>')">修改</a>
...
<asp:DataList id="File_List" RepeatColumns="1" RepeatDirection="Horizontal" runat="server" border=0 RepeatLayout="Table"  cellspacing=0 cellpadding=0 OnItemDataBound="Data_Bound"  OnDeleteCommand="Data_Delete">
   <ItemTemplate>
    序号：<asp:TextBox id="Tb_xuhao" Text='<%#DataBinder.Eval(Container.DataItem,"xuhao")%>' Runat="server" style="width:25px;height:16px;border:1px solid #999999" Maxlength="5"/> 名称：<asp:TextBox id="Tb_title" Text='<%#DataBinder.Eval(Container.DataItem,"title")%>' Runat="server" style="width:160px;height:16px;border:1px solid #999999" Maxlength="50" /> 路径：<a href="<%#DataBinder.Eval(Container.DataItem,"url")%>" target="url"><%#DataBinder.Eval(Container.DataItem,"url")%></a>&nbsp;&nbsp;<a href="javascript:Edit_File('<%#DataBinder.Eval(Container.DataItem,"id")%>','<%#DataBinder.Eval(Container.DataItem,"thetable")%>','<%#DataBinder.Eval(Container.DataItem,"field")%>','<%=FieldType%>','<%#DataBinder.Eval(Container.DataItem,"detail_id")%>','<%=SiteId%>')">修改</a>
    <asp:Label id="Lb_Id" Text='<%#DataBinder.Eval(Container.DataItem,"id")%>' runat="server" Visible="false" />
    <asp:Label id="Lb_Thumbnail" Text='<%#DataBinder.Eval(Container.DataItem,"thumbnail")%>' runat="server" Visible="false" />
    <asp:Label id="Lb_Url" Text='<%#DataBinder.Eval(Container.DataItem,"url")%>' runat="server" Visible="false" />
    <asp:Label id="Lb_UserName" Text='<%#DataBinder.Eval(Container.DataItem,"username")%>' runat="server" Visible="false" />
