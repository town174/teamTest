...
string Pics="";
int News_Pic=0;
string video,fj,author,InforId;
InforId="0";
DataTable dt,dt1;
dt=Get_Data(); 
DataRow dr,dr1; 
...
<li class="info">
已有<span id="clicks"></span>人阅读 &nbsp;-&nbsp; 
<%=((DateTime)dr["thedate"]).ToString("yyyy.MM.dd")%> &nbsp;-&nbsp; 
<a href="<%=Sort_Url(Site_Id,"article",int.Parse(dr["sort_id"].ToString()))%>"><%=Sort_Name(int.Parse(dr["sort_id"].ToString()))%></a> &nbsp;-&nbsp; 
<%=author%>
<%=dr["pa_source"].ToString()==""?"":" &nbsp;-&nbsp;  "+dr["pa_source"].ToString()%>
</li>
</ul>
<%//图片组开始
if(News_Pic>0)
...
%>
<ul class="article_pn">
<li class="prev"><%=_Previous==""?"":"Prev："+_Previous%></li>
<li class="next"><%=_Next==""?"":"Next："+_Next%></li>
</ul>
<!--兼容版，可保证页面完全兼容-->
<div id="SOHUCS" sid="article_<%=InforId%>"></div>
<script>
  (function(){
    var appid = 'cyrxbtLv1',
    conf = 'prod_d6664759a5986b51746359b0d99593cd';
    var doc = document,
